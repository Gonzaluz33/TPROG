...
function cargarOfertasAPostular() {
    mainContentData.forEach(data => {
        const etiquetasHTML = data.keywords.map(item => `#${item.replace(/\s+/g, '')}`)
        const keywords = etiquetasHTML.map(etiqueta => `<b>${etiqueta}</b>`).join('');
        const contentDiv = `
       <div class="d-flex p-2 border mt-2 border-dark align-items-center mb-2">
                  <div style="width: 25%;">
                    <img class="w-75" src="${data.image}" alt="">
                  </div>
                  <div class="w-75">
                    <div class="d-flex flex-column">
                        ...
                        <div class="d-flex flex-column mb-2">
                            <b>Departamento:  ${data.departamento}</b>
                            <b>Ciudad:  ${data.ciudad}</b>
                            <b>Remuneración:  ${data.remuneracion}</b>
                            <b>Fecha de Alta:  ${data.fechaAlta}</b>
                            <b>Horario:  ${data.horario}</b>
                        </div>
                        <div class="d-flex gap-1 justify-content-start">
                            ${keywords}
                        </div>
                        <div class="d-flex justify-content-end">
                            <a href="ingresarPostulacion.html" class="text-dark">Postular</a>
                        </div>
                    </div>
                  </div>
                </div>
    `;
        $('#mainOfertasAPostular').append(contentDiv);
    });
}

function obtenerDatosOferta(id){
    let data = mainContentData.find((item) => item.id ===1);
    const contentDiv = `
       <div class="d-flex p-2 border mt-2 border-dark align-items-center mb-2">
                  <div style="width: 25%;">
                    <img class="w-75" src="${data.image}" alt="">
